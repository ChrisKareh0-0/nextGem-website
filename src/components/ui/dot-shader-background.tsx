...
const DotMaterial = shaderMaterial(
  ...
  dotColor: new THREE.Color('#14ee25'),
...
function Scene() {
  ...
  useEffect(() => {
    // Guard access to uniforms — shaderMaterial creates a 'uniforms' object but TS
    // may not be able to prove its shape. Cast to any and only set when present.
    const uniforms = (dotMaterial as any).uniforms
    if (uniforms) {
      try {
      ...
      if (typeof uniforms.dotOpacity !== 'undefined') {
  ...
  useFrame((state) => {
    const uniforms = (dotMaterial as any).uniforms
    if (uniforms && typeof uniforms.time !== 'undefined') {
